...
爬取 104 人力銀行的職缺搜尋 API，篩選近期刊登的職缺，並可將結果輸出成表格 / JSON / CSV，或彙整前 N 筆傳送到 LINE Notify。
...
- 每天定時（`daily-scrape.yml`）自動爬取並透過 LINE Notify 推播最新職缺。
...
internal/notifier    LINE Notify 推播
...
| `--line-token` | 設定後會把前 N 筆結果推播到 LINE Notify |
...
`daily-scrape.yml` 每天 01:00 UTC（台灣時間 09:00）自動執行，也可以手動 `workflow_dispatch` 並自訂 `keyword` / `months` / `pages`。流程：checkout → 安裝 Playwright Chromium → build → 執行爬蟲 → 把前 10 筆結果推播到 LINE → 把完整結果 `jobs.json` 存成 30 天效期的 Artifact。

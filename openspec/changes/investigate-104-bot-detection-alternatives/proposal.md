...
- **No behavioral rewrite of `internal/client` as part of this change** — this is investigation/decision-making only. It does add two small, additive, TDD'd, opt-in instrumentation pieces needed to gather real evidence, with default behavior unchanged: (1) `isCloudflareChallenge`/`ErrCloudflareChallenge` so a block is an explicit error instead of a 60s timeout, and (2) a `SCRAPER_BROWSER_CHANNEL` env var (default: unset, unchanged bundled-Chromium behavior) so option B (real Chrome channel) could actually be spiked against production rather than assessed on paper. The actual chosen-alternative rewrite is still a separate follow-up change.

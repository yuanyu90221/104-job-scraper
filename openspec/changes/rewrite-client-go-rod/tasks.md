...
- [x] 6.5 Dispatch `daily-scrape.yml` once on the real GitHub-hosted runner to confirm the go-rod build behaves at parity with the playwright-go baseline (pass/fail parity is success — this spike is not expected to fix IP-reputation blocking) — dispatched run 28715484675: the go-rod scrape itself succeeded (22 jobs found, no Cloudflare block), better than the two immediately-prior playwright-go baseline runs (28714047350, 28713721389) which were both blocked by Cloudflare. The job's overall exit code was still 1, but only because of a separate, unrelated failure in the downstream LINE Notify step (`dial tcp: lookup notify-api.line.me: no such host`) — confirmed via direct DNS lookup that `notify-api.line.me` is NXDOMAIN everywhere, consistent with LINE's LINE Notify service having been officially decommissioned (retired 2025-03-31); this is a pre-existing, go-rod-unrelated breakage worth tracking separately.
